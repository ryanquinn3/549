...
void* FSM1(void* arg)
{
	...
	while(1)
	{
		...
		//DUMMY VIDEO TEST - REPLACE THIS WITH PYTHON SCRIPT FOR FULL TEST
		system("sleep 2");
		//system(video_command.c_str());



		//CREATE Test_X_done.txt FOR PIUI CONTINUATION
...
void* FSM2(void* arg)
{
	...
	while(1)
	{
		...
		{	
			...
			cout << "Waiting on basestation results file" << endl;
			while(!video.is_open())
